...
export async function POST(req: NextRequest) {
  ...
  await fetch("https://api.brevo.com/v3/contacts", {
    ...
    body: JSON.stringify({
      email,
      attributes: { FIRSTNAME: firstName || "", SMS: phone || "", ADDRESS: address || "", CITY: city || "", STATE: state || "", POSTCODE: zip || "", SOURCE: "beyondhumanity.xyz" },
  ...
  if (CLOSE_API_KEY) {
    await fetch("https://api.close.com/api/v1/lead/", {
      ...
      body: JSON.stringify({
        ...
        contacts: [{ name: firstName || email, emails: [{ email, type: "office" }], phones: phone ? [{ phone, type: "mobile" }] : [], addresses: address ? [{ address_1: address, city: city || "", state: state || "", zipcode: zip || "", country: "US", type: "mailing" }] : [] }],
